									...
									<div class="td-title">
										<div>
											<i
												...
												:class="`zk-table--tree-icon zk-icon ${
													model.isFold ? 'zk-icon-minus-square-o zk-shield' : 'zk-icon-plus-square-o'
												}`"
												...
												<div
													...
													<template>
														<span
															...
															:style="{'width': column.expandFunc ? '40px' : 'auto'}"
															class="node-title"
														>
														{{model[column.fieldName]}}<slot
															v-if="column.mainAccountSlotName && model.level === 1"
														...
														<div
															v-if="column.expandFunc && model[tableListName] && model[tableListName].length > 0"
															class="zk-table__cell-inner zk-table--expand-inner"
															:class="{'zk-table--expanded-inner': model.isExpand}"
											...
											<!--纯表格-->
											<div v-show="model.isExpand && model[tableListName].length > 0" class="extend-table" :style="tableRowRender">
												<span class="level-line" v-show="model.isFold && model.isExpand && model.children.length > 0"></span>
												<slot
                          name="extendTable"
                          :data="model"
                        ></slot>
...
<script>
  export default {
    name: 'treeUnit',
				props: ['model', 'num', 'nodes', 'root', 'trees', 'treeColumnList', 'columnList', 'tableListName'],
				data() {
					return {
				...
				methods: {
					...
					treeColumnRender(column, model) {
						return {
							'flex-basis': column.key !== 2 ? (column.minWidth - 14 * (model.level - 1) + 'px') : (column.minWidth + 13 * (model.level - 1) + 'px'), 
							'min-width': column.key !== 2 ? column.minWidth - 14 * (model.level - 1) + 'px': (column.minWidth + 13 * (model.level - 1) + 'px'),
